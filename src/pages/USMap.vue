...
function toggleActive(action) {
  if (action === "remove") {
    const elm = document.getElementById(activeState.value);
    if (elm) {
...
function showModal({ target }) {
  ...
  const data = states.find((x) => x.code === id);
...
<template>
  <section class="q-pa-md row items-start q-gutter-md">
    ...
    <q-card class="col fit">
      <q-card-section>
        <div class="map relative-position">
          <!-- popup -->
          <transition
            appear
          ...
          >
            <div v-show="modal">
              <q-card
                bordered
                class="absolute-center shadow-24"
                style="
                  background-image: linear-gradient(to right, #59aee1, #1fa1db);
                  color: #ffffff;
                "
              >
                <q-btn
                  ...
                  class="absolute-top-right q-mb-lg shadow-24"
                  @click="hideModal"
                />
                <q-card-section>
                  <div>{{ state }}, {{ stateData }}</div>
                </q-card-section>
              </q-card>
            </div>
          </transition>
          ...
          <div>
            <svg
              ...
              version="1.1"
              viewBox="0 0 917.59998 585.59998"
              class="state-map"
              id="state-map"
            >
              <g @click="showModal">
...
.map {
  ...
  $State-hover: lighten($primary, 10%);
  .state-map {
    circle.active {
      fill: $State-hover;
    }
